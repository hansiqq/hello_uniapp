<template>
  <div>
    <div class="form_picker_title" @click="show()" v-if="title!==''">
      <div class="picker_title-left">{{title}}</div>
      <div>{{pickerModel}}</div>
      <div>
        <x-icon name="icon-zk-jiantou" :color="'#909399'" size="15"></x-icon>
      </div>
    </div>
    ...
    methods: {
      init () {
        if (this.pickerValueDefault.length !== 0) {
          this.pickerModel = this.pickerValueArray[this.pickerValueDefault[0]].label
        }
      },
      ...
      pickerConfirm (e) {
        ...
        this.pickerModel = pickObj.label
        var value = [pickObj.value[0]]
      ...
      _getPickerLabelAndValue (value, mode) {
        ...
        if (mode === 'selector') {
          ...
          pickerLable =
            this.deepLength === 2 ? `${this.pickerValueMulTwoOne[value[0]].label}-${this.pickerValueMulTwoTwo[value[1]].label}` : `${this.pickerValueMulThreeOne[value[0]].label}-${this.pickerValueMulThreeTwo[value[1]].label}-${this.pickerValueMulThreeThree[value[2]].label}`
          if (this.deepLength === 2) {
