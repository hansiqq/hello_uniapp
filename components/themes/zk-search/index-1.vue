  ...
  <view class="zk-search-a">
    <view class="zk-head-logo">
      <view class="search_icon_left">
        ...
        <i class="icon iconfont zk-arrows-left_black" v-show="shows" @click="back()" v-if="isNotApp"></i>
      </view>
...
export default {
  ...
  methods: {
    ...
    toCart() {
      ala.to('/pages/index?path=order_cart')
    },
    back() {
      ala.back()
    },
    async init() {
      ...
      if (!ala.strIsEmpty(this.inputModel)) {
    ...
    searchConfirm(e) {
      ala.to('/pages/index?path=product_list&Keyword=' + e.detail.value)
    },
    searchReturn() {},
    search() {
      ala.to('/pages/index?path=product_list&Keyword=' + this.searchMdel)
    },
    getCart() {
      ala.to('/pages/index?path=order_cart')
    }
