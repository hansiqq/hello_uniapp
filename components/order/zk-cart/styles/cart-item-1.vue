                ...
                <view class="zk-cart_right">
                  ...
                  <view class="zk-cart-item_content">
                    ...
                    <div class="item_content-info">
                      ...
                      <div class="buycounts">x{{ productItem.buyCount }}</div>
                    </div>
                  </view>
                </view>
...
export default {
  ...
  methods: {
    ...
    async init() {
      var response = await ala.httpGet('api/cart/getcart')
      ...
      uni.showModal({
        ...
        success: async function(res) {
          if (res.confirm) {
            ...
            var deleteCart = await ala.httpGet('api/cart/removecart', paramenter)
            if (deleteCart) {
              ala.toastSuccess('删除成功')
              vueThis.init()
            } 
          } else if (res.cancel) {
          }
    ...
    sumbit() {
      ...
      if (this.widgetModel.storeItems.length === 0) {
        ala.toastWarn('请先添加商品入购物车')
      } else if (buyProductInfo.length === 0) {
        ala.toastWarn('请先选择商品')
      } else {
        uni.showLoading({
          title: 'loading'
        })
        ala.localSet('buyProductInfo', buyProductInfo)
        ala.to('/pages/index?path=order_buy&isFromCart=true')
      }
    },
    goDetails(id) {
      ala.to('/pages/index?path=product_show&id=' + id)
    }
  }
}
</script>
