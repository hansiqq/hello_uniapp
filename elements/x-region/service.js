...
export default {
  ...
  async initAddress(jsThis) {
    var addressInt = await api.addressAll()
    jsThis.addressModel.provinceData = addressInt.province
    jsThis.addressModel.cityData = addressInt.city
    jsThis.addressModel.areaData = addressInt.area
    ...
    if (!ala.strIsEmpty(jsThis.dataModel)) {
        ...
        one.forEach((two, twoIndex) => {
          two.forEach((three, threeIndex) => {
            if (Number(three.value) === Number(jsThis.dataModel)) {
              ...
              var e = {
                label: jsThis.provinceDataList[oneIndex].label - addressModel.cityData[oneIndex][twoIndex].label - addressModel.areaData[oneIndex][twoIndex][threeIndex].label
              }
  ...
  handPickValueDefault(jsThis) {
    ...
    if (jsThis.pickerValueDefault !== [0, 0, 0]) {
      ...
      if (jsThis.pickerValueDefault[1] > addressModel.cityData[jsThis.pickerValueDefault[0]].length - 1) {
        jsThis.pickerValueDefault[1] = addressModel.cityData[jsThis.pickerValueDefault[0]].length - 1
      }
      if (jsThis.pickerValueDefault[2] > addressModel.areaData[jsThis.pickerValueDefault[0]][jsThis.pickerValueDefault[1]].length - 1) {
        jsThis.pickerValueDefault[2] = addressModel.areaData[jsThis.pickerValueDefault[0]][jsThis.pickerValueDefault[1]].length - 1
      }
    }
  },
  // 监听选择器值变化
  ...
  _getLabel(jsThis) {
    let pcikerLabel = jsThis.provinceDataList[jsThis.pickerValue[0]].label + '-' + jsThis.cityDataList[jsThis.pickerValue[1]].label + '-' + jsThis.areaDataList[jsThis.pickerValue[2]].label
    return pcikerLabel
  },
  _getCityCode(jsThis) {
